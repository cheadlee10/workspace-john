...
const batch='next-queued-email-assets-2026-03-03-batch81.md';
const e=fs.readFileSync(path.join(process.cwd(),'email-templates',batch),'utf8');
...
const queueFiles=fs.readdirSync(path.join(process.cwd(),'email-templates')).filter(f=>/^send-queue-\d{4}-\d{2}-\d{2}-next-batches\.jsonl$/.test(f)).sort();
const trackerFiles=fs.readdirSync(path.join(process.cwd(),'email-templates')).filter(f=>/^send-queue-\d{4}-\d{2}-\d{2}-next-batches-tracker\.csv$/.test(f)).sort();
const qjFile=queueFiles[queueFiles.length-1], qcFile=trackerFiles[trackerFiles.length-1];
const qj=qjFile?fs.readFileSync(path.join(process.cwd(),'email-templates',qjFile),'utf8'):'';
const qc=qcFile?fs.readFileSync(path.join(process.cwd(),'email-templates',qcFile),'utf8'):'';
const missingJsonl=ids.filter(id=>!qj.includes(id));
const missingCsv=ids.filter(id=>!qc.includes(id));

console.log(JSON.stringify({wave,batch,slugs,siteResults,email:{sectionCount:sections.length,emailIssues,ids,queueJsonl:qjFile,queueCsv:qcFile,missingJsonl,missingCsv}},null,2));