...
const wave=fs.readdirSync(path.join(cwd,'sites'))
...
const batch=fs.readdirSync(path.join(cwd,'email-templates'))
  .filter(f=>/^next-queued-email-assets-\d{4}-\d{2}-\d{2}-batch\d+\.md$/.test(f))
  .map(f=>({name:f,num:Number(f.match(/batch(\d+)/)[1])}))
  .sort((a,b)=>b.num-a.num)[0].name;

const e=fs.readFileSync(path.join(cwd,'email-templates',batch),'utf8');
...
const qjPath=path.join(cwd,'email-templates','send-queue-2026-03-02-next-batches.jsonl');
const qcPath=path.join(cwd,'email-templates','send-queue-2026-03-02-next-batches-tracker.csv');
const qj=fs.existsSync(qjPath)?fs.readFileSync(qjPath,'utf8'):'';
const qc=fs.existsSync(qcPath)?fs.readFileSync(qcPath,'utf8'):'';
const missingJsonl=ids.filter(id=>!qj.includes(id));
const missingCsv=ids.filter(id=>!qc.includes(id));

console.log(JSON.stringify({wave,batch,slugs,siteResults,email:{sectionCount:sections.length,emailIssues,ids,missingJsonl,missingCsv}},null,2));