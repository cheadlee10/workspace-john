...
# Parse local datetime string used in artifacts: YYYY-MM-DD HH:MM
FMT = "%Y-%m-%d %H:%M"
...
for lid in TARGET_IDS:
    s = sched_by_id[lid]
    # normalize window/gate/send fields from queue source-of-truth
    q = queue_by_id[lid]
    for f in [
...
tracker["updated_at_local"] = "2026-03-03 09:10:00"
tracker["updated_by"] = "subagent-watchdog72-sendops-queueing"

TRACKER_PATH.write_text(json.dumps(tracker, indent=2) + "\n", encoding="utf-8")
print("watchdog72 normalization enforced for", ", ".join(TARGET_IDS))
