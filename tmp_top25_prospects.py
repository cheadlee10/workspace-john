...
for i,o in enumerate(rows[:25],1):
    print(json.dumps({
        'rank':i,'id':o.get('id'),'score':o.get('_score'),'source':o.get('source'),'client':o.get('client'),
        'service':o.get('service'),'estimated_value':o.get('estimated_value'),'date':o.get('date'),'url':o.get('url',''),
        'next_action':'Upwork proposal now' if 'upwork' in (o.get('source','').lower()) else 'Direct outreach',
        'notes':o.get('notes','')[:180]
    }))
