...
root = Path('.')
email_md = root/'email-templates'/'next-queued-email-assets-2026-03-03-batch105.md'
...
items = []
for sec in sections:
    m = re.search(r'^##\s+\d+\)\s+(.+?)\s+\(`(nosite-\d+)`\)', sec, re.M)
    if not m:
        continue
    biz = m.group(1).strip()
    lid = m.group(2)
...
# base schedule from max scheduled timestamp in existing file
existing_times = []
for r in rows:
...
for lid, biz in items:
    ...
    row = {
        ...
        "priority_tier": "P2",
        "priority_score": 79,
        "template_asset_file": "next-queued-email-assets-2026-03-03-batch105.md",
        "asset_ready": True,
...
for lid, biz in items:
    ...
    if not r:
        continue
    vals = [
        r['scheduled_at'], 'BATCH-K-WAVE6', '1', 'email', lid, biz, '', 'queued', 'pending', 'none', 'none',
        ...
        'asset=next-queued-email-assets-2026-03-03-batch105.md;approval=pending_main_agent_approval;verification=pending_contact_enrichment'
    ]
    body.append(vals)

# stable sort by timestamp then lead id
