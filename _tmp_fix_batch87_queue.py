...
batch_file = Path('email-templates/next-queued-email-assets-2026-03-03-batch87.md')
...
lead_ids = sorted(set(re.findall(r'`(wave\d+-\d{3})`', email)), key=lambda x: int(x.split('-')[1]))
...
with csv_path.open('a', encoding='utf-8', newline='') as f:
    ...
    for obj in new_json_objs:
        ...
        if len(row) != len(header):
            # conservative fallback: pad or trim
            if len(row) < len(header):
...
print(f'Appended {len(new_json_objs)} JSONL entries and up to {len(new_json_objs)} CSV rows for batch87.')
