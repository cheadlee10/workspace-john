...
pairs=re.findall(r'##\s+\d+\)\s+(.+?)\s+\(`(wave6-\d+)`\)',et)
lead_ids=[lead for _,lead in pairs]
sections=[]
for i,(_,lid) in enumerate(pairs):
    start=et.find(f'(`{lid}`)')
    # slice from this heading to next heading
email_results=[]
for _,lid in pairs:
    m=re.search(rf'##\s+\d+\).*?\(`{re.escape(lid)}`\)([\s\S]*?)(?=\n---\n\n##\s+\d+\)|\Z)',et)
...
with csvf.open(encoding='utf-8',newline='') as f:
    r=list(csv.DictReader(f)); csv_ids={x.get('lead_id') for x in r}
