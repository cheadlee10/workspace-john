...
asset = "next-queued-email-assets-2026-03-03-batch92.md"
...
pattern = re.compile(r"##\s+\d+\)\s+(.+?)\s+\(`(sprint-\d{8}-\d{3})`\)")
items = pattern.findall(text)
if not items:
    raise SystemExit("No batch items found")

existing = jsonl_path.read_text(encoding="utf-8")
new_items = [(name, lid) for name, lid in items if lid not in existing]
if not new_items:
    print("No missing leads to append")
    raise SystemExit(0)

last_obj = json.loads([ln for ln in existing.splitlines() if ln.strip()][-1])
