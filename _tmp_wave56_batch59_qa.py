...
wave=root/'sites/premium-v3-wave56'
...
for f in files:
    ...
    for m in re.finditer(r"<(input|textarea|select)\b([^>]*)>",t,re.I):
        ...
        if idm and idm.group(1) in labels: continue
...
email=(root/'email-templates'/'next-queued-email-assets-2026-03-03-batch59.md').read_text(encoding='utf-8')
...
for i,b in enumerate(email_bodies,1):
    print('EMAIL',i,'live', '{{live_url}}' in b,'shot','{{screenshot_url}}' in b)
